...
def get_strom_timeseries(
    ...
    for date in date_range:
        url = BASE_URL.format(year=date.year, month=date.month, day=date.day, price_area=price_area)
        try:
...
def main():
    ...
    try:
        start_time_ = (
            start_time
            if start_time
            else get_last_timestamp_in_metric(metric_name)
        )
        ...
        logging.info(
            f"Fetching electricity prices from '{start_time_}' to '{end_time}'"
        )
